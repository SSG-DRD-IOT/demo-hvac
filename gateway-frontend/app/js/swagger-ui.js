	...
	.service('swaggerClient', ['$q', '$http', function($q, $http) {
		...
		this.send = function(swagger, operation, values, transform) {
			...
			var request = {
					...
					url: [swagger.schemes && swagger.schemes[0] || 'http', '://', swagger.host, path].join(''),
					headers: headers,
					data: values.body,
					params: query
				},
